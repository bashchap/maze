...
BASE_ORIGIN_X = int( GRID_WIDTH / 2 - (GRID_BOX_X / 2) )
BASE_ORIGIN_Y = int( GRID_HEIGHT / 2 - (GRID_BOX_Y / 2) )
...
# Create data for animation (4D frame) in a different file
...
for ORIGIN_Z in range(BASE_ORIGIN_Z, BASE_ORIGIN_Z + GRID_BOX_Z * 2, GRID_Z_INC_MIN):
    print(f"> Progress: Frame {frame} : {100 / FRAME_MAX * frame:3.4f}\%")
    walk_the_grid()
    frame += 1

PLOTDATA2D_FILE.close()
PLOTDATA3D_FILE.close()
PLOTDATA4D_FILE.close()
print_totals()